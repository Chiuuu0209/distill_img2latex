...
class PositionalEncoding2D_Light(nn.Module):
    """2-D positional encodings for the feature maps produced by the encoder.
    ...
    def __init__(self, d_model: int, max_h: int = 2000, max_w: int = 2000) -> None:
        ...
        self.pe_h, self.pe_w = self.make_pe(d_model, max_h, max_w)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        hhh = len(x[0][0])
        www = len(x[0][0][0])
        assert x.shape[1] // 2 == self.pe_h.shape[2]
        pe_h = self.pe_h[: hhh].permute(2, 0, 1).expand(-1, -1, www)
        
        pe_w = self.pe_w[: www].permute(2, 1, 0).expand(-1, hhh, -1)
        dynamic_pe = torch.cat([pe_h, pe_w], dim=0)  # (d_model, H, W)
        x = x + dynamic_pe.cuda()
        return x


class PositionalEncoding1D_Light(nn.Module):
    """Classic Attention-is-all-you-need positional encoding."""

    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 5000) -> None:
        super().__init__()
        self.dropout = nn.Dropout(p=dropout)
        self.pe = self.make_pe(d_model, max_len)  # (max_len, 1, d_model)
